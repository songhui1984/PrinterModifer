...
public class Script implements Code
{
	...
	public Script(Words ws, boolean main)
	{
		...
		List<Words> line = Syntax.split(ws);
		if (line.size() == 1)
		{
...
//			System.out.println("code:-----------》"+code);
		...
		{
			Iterator<Words> iter = line.iterator();
			while (iter.hasNext())
			{
				Code code = Syntax.sentenceOf(iter.next());
...
//				return with;
	...
	public String toText(String space)
	{
		...
		while (iter.hasNext())
		{
			buf.append(space + (iter.next()).toText(space) + (iter.hasNext() ? "\n" : ""));
		}
